...
class Robot1HardwareInterface(Node):
  def __init__(self):
    super().__init__("robot1_hardware_interface")
    ...
    self.joint_hardware_objectives_subscriber = self.create_subscription(
      JointState, "/robot1/joint_hardware_objectives", 
    ...
    self.current_joint_state.name = ["shoulder_joint_r1", "arm_joint_r1", "forearm_joint_r1"]
    ...
    self.get_logger().info("Robot 1 - Hardware Interface inicializada")
...
def main(args=None):
  try:
    ...
    node = Robot1HardwareInterface()
    rclpy.spin(node)
  except KeyboardInterrupt as e:
    print("Robot 1 - Hardware Interface stopped")
  finally: 
    node.destroy_node()
    rclpy.shutdown()
