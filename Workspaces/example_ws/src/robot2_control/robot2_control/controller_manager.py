...
class Robot2ControlManager(Node):
  def __init__(self):
    super().__init__("robot2_control_manager")
    ...
    self.hardware_command_publisher = self.create_publisher(
      JointState, "/robot2/joint_hardware_objectives", 10
    )
    
    self.get_logger().info("Robot 2 - Control Manager inicializado")
...
def main(args=None):
  try:
    ...
    node = Robot2ControlManager()
    rclpy.spin(node)
  except KeyboardInterrupt as e:
    print("Robot 2 - Control Manager stopped")
  finally: 
    node.destroy_node()
    rclpy.shutdown()
